	...
	window.addEventListener('scroll', function(e) {
		document.querySelectorAll('.parallax').forEach(function(target){
			...
			target.childNodes.forEach(function(child) {
				...
				child.style['top'] = -(target.getBoundingClientRect().top * (target.getAttribute('intensity') || 0.2)) + 'px';
			});
...
//NavBar
	...
	window.addEventListener('scroll', function(e) {
		...
		if (scrollElement.scrollTop == 0 && navBar.classList.contains('active')) navBar.classList.remove('active');
		if (scrollElement.scrollTop > 0 && !navBar.classList.contains('active')) navBar.classList.add('active');
	});
